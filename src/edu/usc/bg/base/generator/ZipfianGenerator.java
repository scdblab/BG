...
{     
	public static final double ZIPFIAN_CONSTANT=0.7;
	public static Vector<Double> probabilities = new Vector<Double>(); //seroted desc
	...
	static double zetastatic(long st, long n, double theta, double initialsum)
	{
		...
		for (long i=st; i<n; i++)
		{
			...
			probabilities.add(1/(Math.pow(i+1,theta)));
		}
		
		//System.out.println("countforzeta="+countforzeta);
		for(int i=0; i<probabilities.size(); i++){
			//System.out.println("before**"+probabilities.get(i));
			double actProb = ((double)(probabilities.get(i)))/sum;
			probabilities.setElementAt(actProb, i);
			//System.out.println("after**"+probabilities.get(i));
		}
	...
	public long nextLong(long itemcount)
	{
		...
		if (itemcount!=countforzeta)
		{
			...
			synchronized(this)
			{
				if (itemcount>countforzeta)
				{
					//System.out.println("WARNING: Incrementally recomputing Zipfian distribtion. (itemcount="+itemcount+" countforzeta="+countforzeta+")");
				...
				else if ( (itemcount<countforzeta) && (allowitemcountdecrease) )
				{
					...
					System.out.println("WARNING: Recomputing Zipfian distribtion. This is slow and should be avoided. (itemcount="+itemcount+" countforzeta="+countforzeta+")");
