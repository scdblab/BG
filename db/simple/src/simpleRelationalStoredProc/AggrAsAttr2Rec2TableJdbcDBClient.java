 ...
 *
 */
public class AggrAsAttr2Rec2TableJdbcDBClient extends DB implements JdbcDBClientConstants {
	...
	public int queryConfirmedFriendshipIds(int profileId, Vector<Integer> confirmedIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT inviteeid from confirmedfriendship where friend1="+profileId+" ";
			rs = st.executeQuery(query);
			while(rs.next()){
	...
	public void createSchema(Properties props){
		...
		try {
			...
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP ADD CONSTRAINT CONFRMEDFRIENDSHIP_PK PRIMARY KEY (FRIEND1, FRIEND2) ENABLE");
			...
			stmt.execute("CREATE OR REPLACE PROCEDURE INSERTFRIEND " +
							"("+ 
						         " P_INVITERID   IN CONFIRMEDFRIENDSHIP.FRIEND1%TYPE    , "+
						         " p_INVITEEID     IN CONFIRMEDFRIENDSHIP.FRIEND1%TYPE  "+
						     ")" +
						"AS "+ 
						"BEGIN "+ 
						 "INSERT INTO CONFIRMEDFRIENDSHIP "+
   ...
   @Override
	public void buildIndexes(Properties props) {
		...
		try {
			...
			stmt.executeUpdate("CREATE INDEX CONFIRMEDFRIENDSHIP_INVITEEID ON CONFIRMEDFRIENDSHIP (FRIEND1)"
					+ "COMPUTE STATISTICS NOLOGGING");
			...
			stmt.executeUpdate("CREATE INDEX CONFIRMEDFRIENDSHIP_INVITERID ON CONFIRMEDFRIENDSHIP (FRIEND2)"
					+ "COMPUTE STATISTICS NOLOGGING");
