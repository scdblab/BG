 ...
 *
 */
public class SimpleMultiTableStoredProcJdbcDBClient extends DB implements JdbcDBClientConstants {
	...
	private static int DELCMT_STMT = 19;
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			//friend count
			query = "SELECT count(*) FROM  confirmedfriendship WHERE (inviterID = ? )";
			//preparedStatement = conn.prepareStatement(query);
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		try {
			if(insertImage && FSimagePath.equals("")){
				query = "SELECT userid, inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM users, confirmedfriendship WHERE  (inviterid=? and userid=inviteeid)";
				...
				query = "SELECT userid, inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM users, confirmedfriendship WHERE (inviterid=? and userid=inviteeid)";
	...
	public int CreateFriendship(int memberA, int memberB) {
		...
		try {
			...
			String query = "INSERT INTO confirmedfriendship values(?,?)";
			preparedStatement = conn.prepareStatement(query);
			...
			if(preparedStatement != null)
				preparedStatement.close();
			query = "INSERT INTO confirmedfriendship values(?,?)";
			preparedStatement = conn.prepareStatement(query);
	...
	public int thawFriendship(int friendid1, int friendid2){
		...
		String query = "DELETE FROM confirmedfriendship WHERE (inviterid=? and inviteeid= ?) OR (inviterid=? and inviteeid= ?)";
		try {
			//preparedStatement = conn.prepareStatement(query);
	...
	public HashMap<String, String> getInitialStats() {
		...
		try {
			...
			query = "select count(*) from confirmedfriendship where inviteeid="+Integer.parseInt(offset)  ;
			rs = st.executeQuery(query);
			if(rs.next()){
	...
	public int queryConfirmedFriendshipIds(int profileId, Vector<Integer> confirmedIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT inviteeid from confirmedfriendship where inviterid="+profileId+" ";
			rs = st.executeQuery(query);
			while(rs.next()){
	...
	public void createSchema(Properties props){
		...
		try {
			...
			dropTable(stmt, "friendship"); //added in case of back to back experiments
			...
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP ADD CONSTRAINT CONFFRIENDSHIP_PK PRIMARY KEY (INVITERID, INVITEEID) ENABLE");
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP MODIFY (INVITERID NOT NULL ENABLE)");
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP MODIFY (INVITEEID NOT NULL ENABLE)");
			stmt.executeUpdate("ALTER TABLE PENDINGFRIENDSHIP ADD CONSTRAINT PENDFRIENDSHIP_PK PRIMARY KEY (INVITERID, INVITEEID) ENABLE");
			...
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP ADD CONSTRAINT CONFFRIENDSHIP_USERS_FK1 FOREIGN KEY (INVITERID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
			stmt.executeUpdate("ALTER TABLE CONFIRMEDFRIENDSHIP ADD CONSTRAINT CONFFRIENDSHIP_USERS_FK2 FOREIGN KEY (INVITEEID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
			stmt.executeUpdate("ALTER TABLE PENDINGFRIENDSHIP ADD CONSTRAINT PENDFRIENDSHIP_USERS_FK1 FOREIGN KEY (INVITERID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
			stmt.executeUpdate("ALTER TABLE PENDINGFRIENDSHIP ADD CONSTRAINT PENDFRIENDSHIP_USERS_FK2 FOREIGN KEY (INVITEEID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
   ...
   @Override
	public void buildIndexes(Properties props) {
		...
		try {
			...
			dropIndex(stmt, "CONFFRIENDSHIP_INVITEEID");
			dropIndex(stmt, "CONFFRIENDSHIP_INVITERID");
			dropIndex(stmt, "PENDFRIENDSHIP_INVITERID");
			dropIndex(stmt, "PENDFRIENDSHIP_INVITEEID");
			...
			stmt.executeUpdate("CREATE INDEX CONFFRIENDSHIP_INVITEEID ON CONFIRMEDFRIENDSHIP (INVITEEID)"
					+ "COMPUTE STATISTICS NOLOGGING");
			
			stmt.executeUpdate("CREATE INDEX PENDFRIENDSHIP_INVITEEID ON PENDINGFRIENDSHIP (INVITEEID)"
					+ "COMPUTE STATISTICS NOLOGGING");
			...
			stmt.executeUpdate("CREATE INDEX CONFFRIENDSHIP_INVITERID ON CONFIRMEDFRIENDSHIP (INVITERID)"
					+ "COMPUTE STATISTICS NOLOGGING");
			
			stmt.executeUpdate("CREATE INDEX PENDFRIENDSHIP_INVITERID ON PENDINGFRIENDSHIP (INVITERID)"
					+ "COMPUTE STATISTICS NOLOGGING");
