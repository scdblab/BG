...
import relational.JdbcDBClientConstants;
...
public class JdbcDBClient extends DB implements JdbcDBClientConstants {
	private static boolean noDataStore =false;
	public static long serviceTime = 0;
	private static String FSimagePath = "";
	private boolean initialized = false;
	private Properties props;
	private static final String DEFAULT_PROP = "";
	private HashMap<Integer, PreparedStatement> newCachedStatements;
	...
	public static DBConnPool dbConnPool;
	public static boolean useDBConnPool = false;
	public static boolean init = false;
	public static Semaphore dbConnPoolLock = new Semaphore(1);
	
	private DBConn getDBConn() {
		DBConn dbConn = null;
		if (useDBConnPool) {
			dbConn = dbConnPool.getItem();
			conn = dbConn.getConnection();
			newCachedStatements = dbConn.getCachedStatements();
		}
		return dbConn;
	}
	
	private void checkInDBConn(DBConn dbConn) {
		if (useDBConnPool)
			if (dbConn != null)
				dbConnPool.checkIn(dbConn);
	}
	...
	private boolean initDelegate() throws DBException {
		if (noDataStore)
			return true;
		if (initialized) {
		...
		if (useDBConnPool) {
			if (!init) {
				...
				if (!init)  {
					dbConnPool = new DBConnPool(props, 200);
					init = true;
				}
				dbConnPoolLock.release();
			}
			...
			try {
				...
				newCachedStatements = new HashMap<Integer, PreparedStatement>();
	...
	private PreparedStatement createAndCacheStatement(int stmttype, String query)
		...
		PreparedStatement stmt = newCachedStatements.put(stmttype,
				newStatement);
		if (stmt == null)
			return newStatement;
		else
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		int ret = -1;
		while (true) {
			ret = this.acceptFriendDelegate(inviterID, inviteeID);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int acceptFriendDelegate(int inviterID, int inviteeID) {
		if (noDataStore) {
			try {
	...
	public int insertEntity(String entitySet, String entityPK,
		...
		int ret = -1;
		while (true) {
			ret = this.insertEntityDelegate(entitySet, entityPK, values, insertImage);
			if (ret == -3) {
				values.clear();
				this.cleanup(false);
				try {
	...
	private int insertEntityDelegate(String entitySet, String entityPK,
			HashMap<String, ByteIterator> values, boolean insertImage) {
		if (entitySet == null) {
			return -1;
		}
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		int ret = -1;
		while (true) {
			ret = this.viewProfileDelegate(requesterID, profileOwnerID, result,
	...
	private int viewProfileDelegate(int requesterID, int profileOwnerID,
			HashMap<String, ByteIterator> result, boolean insertImage,
			boolean testMode) {
		if (noDataStore) {
			try {
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		int ret = -1;
		while (true) {
			ret = this.listFriendsDelegate(requesterID, profileOwnerID, fields,
	...
	private int listFriendsDelegate(int requesterID, int profileOwnerID,
			Set<String> fields, Vector<HashMap<String, ByteIterator>> result,
			boolean insertImage, boolean testMode) {
		if (noDataStore) {
			try {
	...
	public int viewFriendReq(int profileOwnerID,
		...
		int ret = -1;
		while (true) {
			ret = this.viewFriendReqDelegate(profileOwnerID, result, insertImage, testMode);
			if (ret == -3) {
				result.clear();
				this.cleanup(false);
				try {
	...
	private int viewFriendReqDelegate(int profileOwnerID,
			Vector<HashMap<String, ByteIterator>> result, boolean insertImage,
			boolean testMode) {
		if (noDataStore) {
			try {
...
//				throw new SQLRecoverableException("fake exception");
//			}
//		}
		this.conn.commit();
	}
	...
	public int rejectFriend(int inviterID, int inviteeID) {
		int ret = -1;
		while (true) {
			ret = this.rejectFriendDelegate(inviterID, inviteeID);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int rejectFriendDelegate(int inviterID, int inviteeID) {
		if (noDataStore) {
			try {
				Thread.sleep(serviceTime);
			} catch (InterruptedException e) {
	...
	public int CreateFriendship(int memberA, int memberB) {
		int ret = -1;
		while (true) {
			ret = this.CreateFriendshipDelegate(memberA, memberB);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int CreateFriendshipDelegate(int memberA, int memberB) {
		int retVal = SUCCESS;
	...
	public int inviteFriend(int inviterID, int inviteeID) {
		int ret = -1;
		while (true) {
			ret = this.inviteFriendDelegate(inviterID, inviteeID);
			if (ret == -3) {
				System.out.println("SQLRRR invite " + inviterID + " " + inviteeID);
				this.cleanup(false);
				try {
					this.init();
				} catch (DBException e) {
...
//		System.out.println("SQLRR invite " + inviterID + " " + inviteeID);
		return ret;
	}

	private int inviteFriendDelegate(int inviterID, int inviteeID) {
	...
	public int thawFriendship(int friendid1, int friendid2) {
		int ret = -1;
		while (true) {
			ret = this.thawFriendshipDelegate(friendid1, friendid2);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int thawFriendshipDelegate(int friendid1, int friendid2) {
		if (noDataStore) {
			try {
				Thread.sleep(serviceTime);
			} catch (InterruptedException e) {
	...
	public int viewTopKResources(int requesterID, int profileOwnerID, int k,
		...
		int ret = -1;
		while (true) {
			ret = this.viewTopKResourcesDelegate(requesterID, profileOwnerID, k, result);
			if (ret == -3) {
				result.clear();
				this.cleanup(false);
				try {
	...
	private int viewTopKResourcesDelegate(int requesterID, int profileOwnerID, int k,
			Vector<HashMap<String, ByteIterator>> result) {
		int retVal = SUCCESS;
		ResultSet rs = null;
	...
	public int getCreatedResources(int resourceCreatorID,
		...
		int ret = -1;
		while (true) {
			ret = this.getCreatedResourcesDelegate(resourceCreatorID, result);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int getCreatedResourcesDelegate(int resourceCreatorID,
			Vector<HashMap<String, ByteIterator>> result) {
		int retVal = SUCCESS;
		ResultSet rs = null;
		Statement st = null;
	...
	public int viewCommentOnResource(int requesterID, int profileOwnerID,
		...
		int ret = -1;
		while (true) {
			ret = this.viewCommentOnResourceDelegate(requesterID, profileOwnerID, resourceID, result);
			if (ret == -3) {
				result.clear();
				this.cleanup(false);
				try {
	...
	private int viewCommentOnResourceDelegate(int requesterID, int profileOwnerID,
			int resourceID, Vector<HashMap<String, ByteIterator>> result) {
		int retVal = SUCCESS;
		ResultSet rs = null;
	...
	public int postCommentOnResource(int commentCreatorID, int profileOwnerID,
		...
		int ret = -1;
		while (true) {
			ret = this.postCommentOnResourceDelegate(commentCreatorID, profileOwnerID, resourceID, commentValues);
			if (ret == -3) {
				commentValues.clear();
				this.cleanup(false);
				try {
	...
	private int postCommentOnResourceDelegate(int commentCreatorID, int profileOwnerID,
			int resourceID, HashMap<String, ByteIterator> commentValues) {
		int retVal = SUCCESS;
	...
	public int delCommentOnResource(int resourceCreatorID, int resourceID,
		...
		int ret = -1;
		while (true) {
			ret = this.delCommentOnResourceDelegate(resourceCreatorID, resourceID, manipulationID);
			if (ret == -3) {
				this.cleanup(false);
				try {
					this.init();
	...
	private int delCommentOnResourceDelegate(int resourceCreatorID, int resourceID,
			int manipulationID) {
		int retVal = SUCCESS;
	...
	public int queryPendingFriendshipIds(int inviteeid,
		...
		int ret = -1;
		while (true) {
			ret = this.queryPendingFriendshipIdsDelegate(inviteeid, pendingIds);
			if (ret == -3) {
				pendingIds.clear();
				this.cleanup(false);
				try {
	...
	private int queryPendingFriendshipIdsDelegate(int inviteeid,
			Vector<Integer> pendingIds) {
		int retVal = SUCCESS;
		Statement st = null;
		ResultSet rs = null;
	...
	public int queryConfirmedFriendshipIds(int profileId,
		...
		int ret = -1;
		while (true) {
			ret = this.queryConfirmedFriendshipIdsDelegate(profileId, confirmedIds);
			if (ret == -3) {
				confirmedIds.clear();
				this.cleanup(false);
				try {
	...
	private int queryConfirmedFriendshipIdsDelegate(int profileId,
			Vector<Integer> confirmedIds) {
		int retVal = SUCCESS;
		Statement st = null;
		ResultSet rs = null;
	...
	public boolean dataAvailable() {
		boolean ret = false;
		while (true) {
			ret = this.dataAvailableDelegate();
			if (!ret) {
				this.cleanup(false);
				try {
					this.init();
	...
	private boolean dataAvailableDelegate() {
		boolean retVal = true;
		Statement stmt = null;
		ResultSet rs = null;
	...
	public int viewProfile(int requestId, int profileId,
			HashMap<String, ByteIterator> result, boolean insertImage,
			boolean b, boolean simplified) {
		if (!simplified) {
			return viewProfile(requestId, profileId, result, insertImage, b);
		} else {
			return viewProfileSimplified(requestId, profileId, result, insertImage, b);
		}
	}

	private int viewProfileSimplified(int requestId, int profileOwnerID,
	...
	public int friendCount(int profileId, StringBuffer result) {
		if (noDataStore) {
			try {
				Thread.sleep(serviceTime);
			} catch (InterruptedException e) {
	...
	public int pendingCount(int profileId, StringBuffer result) {
		if (noDataStore) {
			try {
				Thread.sleep(serviceTime);
			} catch (InterruptedException e) {
		...
		try {
			// friend count
			String query = "SELECT count(*) FROM  friendship WHERE inviteeID = ? AND status = 1 ";

			if ((preparedStatement = newCachedStatements.get(GETPENDCNT_STMT)) == null)
				preparedStatement = createAndCacheStatement(GETPENDCNT_STMT,
						query);
