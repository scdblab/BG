...
public class EhcacheBoostedClient extends DB implements EhcacheClientConstants {
	...
	public boolean init() throws DBException {
		...
		try {
			...
			System.out.println("Starting ehcache with memory=" + MAX_CACHE_MEMORY);
			...
			if (elt != null){
				...
				if ( value != null ){
					...
					for (Map.Entry<String, byte[]> entry : CMEntry.entrySet()) { 
						...
						{
							byte[] pic_array = new byte[entry.getValue().length];
							System.arraycopy( entry.getValue(), 0, pic_array, 0, entry.getValue().length );
							result.put(entry.getKey(), new ObjectByteIterator(pic_array));
						}							
						else
						{
			...
			if(rs.next()){
				for (int i = 1; i <= col; i++){
					...
					if(col_name.equalsIgnoreCase("pic") ){
						...
						CacheEntry.put("resourcecount", rs.getString(col_name).getBytes()) ;
				...
				if (insertImage && !FSimagePath.equals("") ){
					...
					CacheEntry.put("pic", profileImage);
					...
					for (int i=0; i < CMEntry.size(); i++){
						...
						for (Map.Entry<String, byte[]> entry : CacheElt.entrySet()) { 
							...
							if ( entry.getKey().equalsIgnoreCase("tpic") )
							{
								byte[] pic_array = new byte[entry.getValue().length];
								System.arraycopy( entry.getValue(), 0, pic_array, 0, entry.getValue().length );
								resElt.put(entry.getKey(), new ObjectByteIterator(pic_array));
							}
							else
							{
						...
						for (Map.Entry<String, byte[]> entry : CacheElt.entrySet()) { 
							...
							if ( entry.getKey().equalsIgnoreCase("tpic") )
							{
								...
								if(testMode){
									//dump to file
									try{
										FileOutputStream fos = new FileOutputStream(profileOwnerID+"-cacheHit-thumbimage.bmp");
										fos.write(pic_array);
										fos.close();
									}catch(Exception ex){
									}
