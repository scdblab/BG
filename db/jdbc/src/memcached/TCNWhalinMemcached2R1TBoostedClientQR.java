...
public class TCNWhalinMemcached2R1TBoostedClientQR extends DB implements JdbcDBMemCachedClientConstants {
...
//				}
	...
	private String viewProfileQuery(boolean use_image) {
		if (use_image) {
	...
	private String viewProfileQuery(int user_id, boolean use_image) {
		if (use_image) {
			return "SELECT userid,username, fname, lname, gender, dob, jdate, " +
					"ldate, address, email, tel, pic, pendcount, confcount, " +
					"rescount FROM  users WHERE UserID = " + user_id;
		} else {
			return "SELECT userid,username, fname, lname, gender, dob, jdate, " +
					"ldate, address, email, tel, pendcount, confcount, " +
					"rescount FROM  users WHERE UserID = " + user_id;
	...
	private String viewProfileKey(int user_id, boolean use_image) {
		return removeWhitespace(viewProfileQuery(user_id, use_image));
	}
	
	private String listFriendsQuery(boolean use_image) {
		if (use_image) {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
					...
					"WHERE (inviterid=? and userid=inviteeid) and status = 2";
		} else {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
	...
	private String listFriendsQuery(int user_id, boolean use_image) {
		if (use_image) {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel,tpic " +
					"FROM users, friendship " +
					"WHERE (inviterid="+ user_id +" and userid=inviteeid) and status = 2";
		} else {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel " +
					"FROM users, friendship " +
	...
	private String listFriendsKey(int user_id, boolean use_image) {
		return removeWhitespace(listFriendsQuery(user_id, use_image));
	}
	
	private String viewFriendReqQuery(boolean use_image) {
		if (use_image) {
			return "SELECT userid, inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel,tpic " +
					"FROM users, friendship WHERE inviteeid=? and " +
							"status = 1 and inviterid = userid";
		} else {
			return "SELECT userid, inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel " +
					"FROM users, friendship WHERE inviteeid=? and " +
	...
	private String viewFriendReqQuery(int user_id, boolean use_image) {
		if (use_image) {
			return "SELECT userid, inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel,tpic " +
					"FROM users, friendship WHERE inviteeid="+ user_id +" and " +
							"status = 1 and inviterid = userid";
		} else {
			return "SELECT userid, inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel " +
	...
	private String viewTopKKey(int user_id) {
		return removeWhitespace(viewTopKQuery(user_id, TOPK_VALUE));
	}
	
	private String viewCommentQuery() {
		return "SELECT * FROM manipulation WHERE rid = ?";
	}
	
	private String viewCommentQuery(int res_id) {
		return "SELECT * FROM manipulation WHERE rid = " + res_id;
	}
	
	private String viewCommentKey(int res_id) {
...
//					key = "lsFrds:"+inviterID;
					key = listFriendsKey(inviterID, isInsertImage);
...
//					key = "lsFrds:"+inviteeID;
					key = listFriendsKey(inviteeID, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "profile"+inviterID;
					key = viewProfileKey(inviterID, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "profile"+inviteeID;
					key = viewProfileKey(inviteeID, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "viewPendReq:"+inviteeID;
					key = viewFriendReqKey(inviteeID, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "lsFrds:"+friendid1;
					key = listFriendsKey(friendid1, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "lsFrds:"+friendid2;
					key = listFriendsKey(friendid2, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "profile"+friendid1;
					key = viewProfileKey(friendid1, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
...
//					key = "profile"+friendid2;
					key = viewProfileKey(friendid2, isInsertImage);
					if(!common.CacheUtilities.CacheDelete(this.cacheclient, key))
					{
						throw new Exception("Error calling WhalinMemcached delete");
					}
