...
public class ClientForRefreshTechnique extends DB implements JdbcDBMemCachedClientConstants {
	...
	protected static int CACHE_HIT = 0, CACHE_MISS = 1;
	...
	protected String getKeyOwnProfile(int userID) {
		return isInsertImage == true ? "ownprofile" + userID : "ownprofileNoImage" + userID;
	}
	...
	private boolean updateCache(String key, HashMap<String, ByteIterator> result) {
	...
	private boolean updateCache(String key, Vector<HashMap<String, ByteIterator>> result) {
		byte[] payload = CacheUtilities.SerializeVectorOfHashMaps(result);
		try {
			if(!common.CacheUtilities.CacheSet(this.cacheclient, key, payload, this.useTTL, this.TTLvalue, this.compressPayload))
			{
				System.out.println("Error in updateCache with vector of HashMap");
				return false;
			}
	...
	private int updateOwnProfileFriendAccepted(int id) {
		if (id < 0)
			return -1;
		
		String key = getKeyOwnProfile(id);
		...
		HashMap<String, ByteIterator> newValue = new HashMap<String, ByteIterator>();
		...
		while (iterator.hasNext()) {
			...
			value = new String(entry.getValue().toArray());
			
			if (entry.getKey().equals("pendingcount")) {
				currValue = Integer.parseInt(new String(value));
				currValue -= 1;			
				value = currValue + "";
			}
			else if	(entry.getKey().equals("friendcount")) {
				currValue = Integer.parseInt(new String(value));
				currValue += 1;	
				value = currValue + "";
			}	
			
			newValue.put(entry.getKey(), new ObjectByteIterator(value.getBytes()));
		}	
		
		if (updateCache(key, newValue))
			return SUCCESS;			

		System.out.println("Error in OwnProfilePending: update cache id=" + id);
		return -2;		
	}
	
	/** update pending-friend-count on ownprofile */
	private int updateOwnProfilePending(int id, int delta) {
		if (id < 0)
			return -1;
		...
		while (iterator.hasNext()) {
			...
			if (entry.getKey().equals("pendingcount")) {
				int currValue = Integer.parseInt(new String(entry.getValue().toArray()));
				currValue += delta;
				entry.setValue(new ObjectByteIterator((currValue + "").getBytes()));
				
				if (updateCache(key, result))
					return SUCCESS;					
			}
		}	

		System.out.println("Error in OwnProfilePending: update cache id=" + id + " delta=" + delta);
		return -2;
	}
	
	/** update owner profile when a friend is accepted or rejected */
	private int updateOwnProfileFriend(int id, int delta) {
		if (id < 0)
			return -1;
		...
		HashMap<String, ByteIterator> result = new HashMap<String, ByteIterator>();
		if (!CacheUtilities.unMarshallHashMap(result, payload, this.deserialize_buffer)) {
			System.out.println("Error in OwnProfileFriend: Failed to unMarshallVectorOfHashMaps");
			return -1;
		}
		...
		System.out.println("Error in OwnProfileFriend: update cache id=" + id + " delta=" + delta);
		return -2;
	}
	
	/** update profile when a friend is accepted or rejected */
	private int updateProfile(int id, int delta) {
		if (id < 0)
			return -1;
		...
		System.out.println("Error with updateProfile: userid=" + id + ", delta=" + delta);
		return -2;				
	}
	
	/**
	 * When user1 sends friend request to user2, list pending friend of user2 need to be updated
	 * @param inviterID
	 * @param inviteeID
	 * return
	 */
	private int updatePendingFriendsInvite(int inviterID, int inviteeID) {
		...
		System.out.println("Error in updatePendingFriendsInvite: userID=" + inviterID + ", user2id =" + inviteeID);
		return -2;
	} 
	
	private int updatePendingFriendsRemove(int inviterID, int inviteeID) {
		String key = getKeyFriendRequests(inviteeID);
		...
		System.out.println("Error in updatePendingFriendsRemove: inviterID=" + inviterID + ", inviteeID=" + inviteeID);
		return -2;
	}
	
	/**
	 * When user1 sends friend request to user2 and the invitation is accepted, this function updates list friends of user1 by:
	 *    1. get user2 information
	 *    2. get cache value of user 1
	 *    3. updates cache value
	 *    4. set updated value to the cache
	 * @param inviterID
	 * @param inviteeID
	 * return
	 */
	private int updateListFriendsAccepted(int user1, int user2) {		
		HashMap<String, ByteIterator> values = new HashMap<String, ByteIterator>();;
		...
		System.out.println("Error in updatePendingFriendsInvite: userID=" + user1 + ", user2id =" + user2);
		return -2;
	}
	
	private int updateListFriendsRemove(int user1, int user2) {
		String key = getKeyListFriends(user1);
		byte[] payload = common.CacheUtilities.CacheGet(cacheclient, key, this.compressPayload);
		...
		System.out.println("Error in updateListFriendsRemove, user1id=" + user1 + ", user2id=" + user2);
		return -2;
	}
	
	private int resCommentsUpdate(int resID, int commentCreatorID, int profileOwnerID, HashMap<String, ByteIterator> commentValues) {
	...
	private int resCommentRemove(int resCreatorID, int resID, int manipulationID) {
		String key = getKeyCommentsofResource(resID);
		...
		System.out.println("Error in resCommentRemove, resID=" + resID + ", manipulationID=" + manipulationID);
		return -2;
	}
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		...
		try {
		    ...
		    if(!useTTL)
			{
		    	...
		    	updatePendingFriendsRemove(inviterID, inviteeID);
		    	
		    	// update profile
		    	updateProfile(inviterID, 1);
		    	updateProfile(inviteeID, 1);
		    	
		    	// update owner profile	    	
		    	updateOwnProfileFriend(inviterID, +1);
		    	updateOwnProfileFriendAccepted(inviteeID);

		    	// update list friends of inviter and invitee
		    	updateListFriendsAccepted(inviterID, inviteeID);
		    	updateListFriendsAccepted(inviteeID, inviterID);		    	
			}
	...
	public int rejectFriend(int inviterID, int inviteeID) {
		...
		try {
		    ...
		    if (!useTTL)
			{
		    	updateOwnProfilePending(inviteeID, -1);
		    	updatePendingFriendsRemove(inviterID, inviteeID);				
			}
		}catch(SQLException sx){
			retVal = -2;
	...
	public int inviteFriend(int inviterID, int inviteeID) {
		...
		try {
			...
			{
		    	updateOwnProfilePending(inviteeID, 1);
		    	updatePendingFriendsInvite(inviterID, inviteeID);							
			}
		}catch(SQLException sx){
			retVal = -2;
			sx.printStackTrace(System.out);
	...
	public int thawFriendship(int friendid1, int friendid2){
		...
		try {
		    ...
		    if (!useTTL)
			{		
		    	updateOwnProfileFriend(friendid1, -1);
		    	updateOwnProfileFriend(friendid2, -1);
		    	updateProfile(friendid1, -1);
		    	updateProfile(friendid2, -1);
		    	updateListFriendsRemove(friendid1, friendid2);
		    	updateListFriendsRemove(friendid2, friendid1);		    	
			}
		}catch(SQLException sx){
			retVal = -2;
			sx.printStackTrace(System.out);
	...
	public int postCommentOnResource(int commentCreatorID, int profileOwnerID,
		...
		try {
			...
			if (!useTTL)
			{
				resCommentsUpdate(resourceID, commentCreatorID, profileOwnerID, commentValues);
			}
		}catch(SQLException sx){
			retVal = -2;
			sx.printStackTrace(System.out);
	...
	public int delCommentOnResource(int resourceCreatorID, int resourceID,
		...
		try {
			...
			if (!useTTL) {
				resCommentRemove(resourceCreatorID, resourceID, manipulationID);
			}
...
//		System.out.println("Before cache set: " + md5Java(payload));
