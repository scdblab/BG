...
public class WhalinTriggerMemcached2R1TBoostedClientQR extends DB implements JdbcDBClientConstants {
...
//		}
		...
		if (ManageCache) {
			...
			{
				System.out.println("Error. Listener must be specified to manage cache.");
			}
...
//				}
	...
	private static String listFriendsQueryStrPt1(String status, boolean use_image) {
		if (use_image) {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
					...
					"WHERE userid=inviteeid and status = "+ status;
		} else {
			return "SELECT userid,inviterid, inviteeid, username, fname, " +
	...
	private static String listFriendsQueryStrPt2(String id) {
		return " and inviterid=" + id;
	}
...
//					"WHERE userid=inviteeid and status = 2 and inviterid=" + id;
//		}
		return listFriendsQueryStrPt1("2", use_image) + listFriendsQueryStrPt2(id);
	}
	...
	private static String viewFriendReqQueryStrPt1(String status, boolean use_image) {
		if (use_image) {
			return "SELECT userid, inviterid, inviteeid, username, fname, " +
					"lname, gender, dob, jdate, ldate, address,email,tel,tpic " +
	...
	private static String viewFriendReqQueryStrPt2(String id) {
		 return " and inviterid = userid and inviteeid=" + id;
	}
...
//							"status = 1 and inviterid = userid and inviteeid=" + id;
//		}
		return viewFriendReqQueryStrPt1("1", use_image) + viewFriendReqQueryStrPt2(id);
	}
...
//		return stats;
	...
	private void ConstructTriggers(){
		...
		try {
			...
			// ViewProfile
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS00109566866I BEFORE INSERT ON USERS FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  " +
					"KEYTODELETE := CONCAT('"+ viewProfileKey(isInsertImage) +"',:NEW.UserID); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS00109566866D BEFORE DELETE ON USERS FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  " +
					"KEYTODELETE := CONCAT('"+ viewProfileKey(isInsertImage) +"',:OLD.UserID);DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS00109566866U BEFORE UPDATE ON USERS FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  " +
					"IF (:OLD.userid <> :NEW.userid OR :OLD.username <> :NEW.username OR :OLD.fname <> :NEW.fname OR :OLD.lname <> :NEW.lname OR :OLD.gender <> :NEW.gender OR :OLD.dob <> :NEW.dob OR :OLD.jdate <> :NEW.jdate OR :OLD.ldate <> :NEW.ldate OR :OLD.address <> :NEW.address OR :OLD.email <> :NEW.email OR :OLD.tel <> :NEW.tel OR :OLD.pendcount <> :NEW.pendcount OR :OLD.confcount <> :NEW.confcount OR :OLD.rescount <> :NEW.rescount) THEN   " +
						"KEYTODELETE := CONCAT('"+ viewProfileKey(isInsertImage) +"',:OLD.UserID); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END IF; " +
					"IF (:OLD.userid <> :NEW.userid) THEN " +
						"KEYTODELETE := CONCAT('"+ viewProfileKey(isInsertImage) +"',:NEW.UserID); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END IF; " +
					"IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			
			// ListFriends
			stmt.execute("CREATE OR REPLACE PROCEDURE  FRIENDSHIP219717 (arg1 IN FRIENDSHIP.STATUS%TYPE, arg2 IN FRIENDSHIP.INVITERID%TYPE, DELETEKEY IN OUT CLOB) IS " +
					"TOKEN1 CLOB := TO_CLOB('"+ removeWhitespace(listFriendsQueryStrPt1("", isInsertImage))+"');" +
					"TOKEN2 CLOB := TO_CLOB('"+ removeWhitespace(listFriendsQueryStrPt2(""))+"');" +
					"COSAR_Ret_Val BINARY_INTEGER;KEYTODELETE CLOB; BEGIN " +
					"KEYTODELETE := CONCAT(TOKEN1, arg1); KEYTODELETE := CONCAT(KEYTODELETE, TOKEN2); KEYTODELETE := CONCAT(KEYTODELETE, arg2); " +
					"DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE));END;");
			stmt.execute("CREATE OR REPLACE PROCEDURE  USERS219717 (InArg1 IN USERS.USERID%TYPE , DELETEKEY IN OUT CLOB)  IS var1  FRIENDSHIP.STATUS%TYPE; var2  FRIENDSHIP.INVITERID%TYPE;  " +
					"TOKEN CLOB := TO_CLOB('"+ listFriendsKey(isInsertImage) +"'); COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE CLOB;  CURSOR CRS1 IS SELECT FRIENDSHIP.STATUS, FRIENDSHIP.INVITERID FROM FRIENDSHIP WHERE FRIENDSHIP.INVITEEID=InArg1;  BEGIN  OPEN CRS1;  LOOP  FETCH CRS1 into var1, var2;  EXIT WHEN CRS1%NOTFOUND; KEYTODELETE := CONCAT(TOKEN, var2); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END LOOP;  CLOSE CRS1; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219717I BEFORE INSERT ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219717(:NEW.USERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END; ");
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219717I BEFORE INSERT ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219717(:NEW.STATUS, :NEW.INVITERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219717D BEFORE DELETE ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219717(:OLD.USERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;"); 
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219717D BEFORE DELETE ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219717(:OLD.STATUS, :OLD.INVITERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
//			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219717U BEFORE UPDATE ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219717(:OLD.USERID , DELETEKEY); IF (:NEW.USERID <> :OLD.USERID) THEN  USERS219717(:NEW.USERID , DELETEKEY);  END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute(
					"CREATE OR REPLACE TRIGGER  USERS219717U BEFORE UPDATE ON USERS FOR EACH ROW DECLARE     DELETEKEY CLOB; "+
							"                                KEYTODELETE CLOB; "+
							"                                COSAR_RET_VAL BINARY_INTEGER; "+
							"BEGIN "+
							"IF (:OLD.userid <> :NEW.userid OR :OLD.username <> :NEW.username OR :OLD.fname <> :NEW.fname OR :OLD.lname <> :NEW.lname OR :OLD.gender <> :NEW.gender OR :OLD.dob <> :NEW.dob OR :OLD.jdate <> :NEW.jdate OR :OLD.ldate <> :NEW.ldate OR :OLD.address <> :NEW.address OR :OLD.email <> :NEW.email OR :OLD.tel <> :NEW.tel) "+
							"        THEN "+
							"USERS219717(:OLD.USERID , DELETEKEY); "+
							"END IF; "+
							"IF (:NEW.USERID <> :OLD.USERID) "+
							"        THEN  USERS219717(:NEW.USERID , DELETEKEY); "+
							"END IF; "+
							"IF (DELETEKEY != ' ') "+
							"        THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS',      DELETEKEY, 0); "+
							"END IF; "+
					"END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219717U BEFORE UPDATE ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219717(:OLD.STATUS, :OLD.INVITERID , DELETEKEY); IF (:NEW.STATUS <> :OLD.STATUS OR :NEW.INVITERID <> :OLD.INVITERID) THEN   FRIENDSHIP219717(:New.STATUS, :New.INVITERID , DELETEKEY);  END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END; ");

			// ViewPendingRequests 
			stmt.execute("CREATE OR REPLACE PROCEDURE  FRIENDSHIP219817 (arg1 IN FRIENDSHIP.STATUS%TYPE, arg2 IN FRIENDSHIP.INVITEEID%TYPE, DELETEKEY IN OUT CLOB) IS " +
					"TOKEN1 CLOB := TO_CLOB('"+ removeWhitespace(viewFriendReqQueryStrPt1("", isInsertImage)) +"');" +
					"TOKEN2 CLOB := TO_CLOB('"+ removeWhitespace(viewFriendReqQueryStrPt2("")) +"');" +
					"COSAR_Ret_Val BINARY_INTEGER;KEYTODELETE CLOB; BEGIN " +
					"KEYTODELETE := CONCAT(TOKEN1, arg1); KEYTODELETE := CONCAT(KEYTODELETE, TOKEN2); KEYTODELETE := CONCAT(KEYTODELETE, arg2); " +
					"DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE));END;");
			stmt.execute("CREATE OR REPLACE PROCEDURE  USERS219817 (InArg1 IN USERS.USERID%TYPE , DELETEKEY IN OUT CLOB)  IS var1  FRIENDSHIP.STATUS%TYPE; var2  FRIENDSHIP.INVITEEID%TYPE;  " +
					"TOKEN CLOB := TO_CLOB('"+ viewFriendReqKey(isInsertImage) +"'); COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE CLOB;  CURSOR CRS1 IS SELECT FRIENDSHIP.STATUS, FRIENDSHIP.INVITEEID FROM FRIENDSHIP WHERE FRIENDSHIP.INVITERID=InArg1;  BEGIN  OPEN CRS1;  LOOP  FETCH CRS1 into var1, var2;  EXIT WHEN CRS1%NOTFOUND; KEYTODELETE := CONCAT(TOKEN, var2); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END LOOP;  CLOSE CRS1; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219817I BEFORE INSERT ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219817(:NEW.USERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219817I BEFORE INSERT ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219817(:NEW.STATUS, :NEW.INVITEEID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219817D BEFORE DELETE ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219817(:OLD.USERID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219817D BEFORE DELETE ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219817(:OLD.STATUS, :OLD.INVITEEID , DELETEKEY);  IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
//			stmt.execute("CREATE OR REPLACE TRIGGER  USERS219817U BEFORE UPDATE ON USERS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  USERS219817(:OLD.USERID , DELETEKEY); IF (:NEW.USERID <> :OLD.USERID) THEN  USERS219817(:NEW.USERID , DELETEKEY);  END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute(
					"CREATE OR REPLACE TRIGGER  USERS219817U BEFORE UPDATE ON USERS "+
							...
							"IF (:OLD.userid <> :NEW.userid OR :OLD.username <> :NEW.username OR :OLD.fname <> :NEW.fname OR :OLD.lname <> :NEW.lname OR :OLD.gender <> :NEW.gender OR :OLD.dob <> :NEW.dob OR :OLD.jdate <> :NEW.jdate OR :OLD.ldate <> :NEW.ldate OR :OLD.address <> :NEW.address OR :OLD.email <> :NEW.email OR :OLD.tel <> :NEW.tel) "+
							"        THEN  "+
							"USERS219817(:OLD.USERID , DELETEKEY); "+
							"END IF; "+
							"IF (:NEW.USERID <> :OLD.USERID) "+
							"        THEN  USERS219817(:NEW.USERID , DELETEKEY); "+
							"END IF; "+
							"IF (DELETEKEY != ' ') "+
							"        THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS',      DELETEKEY, 0); "+
							"END IF; "+
					"END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  FRIENDSHIP219817U BEFORE UPDATE ON FRIENDSHIP FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; COSAR_RET_VAL BINARY_INTEGER;  BEGIN  FRIENDSHIP219817(:OLD.STATUS, :OLD.INVITEEID , DELETEKEY); IF (:NEW.STATUS <> :OLD.STATUS OR :NEW.INVITEEID <> :OLD.INVITEEID) THEN   FRIENDSHIP219817(:New.STATUS, :New.INVITEEID , DELETEKEY);  END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			
			// ViewTopKResources
			// TODO: this won't work for actual updates. fix the string
			stmt.execute("CREATE OR REPLACE TRIGGER  RESOURCES003891039104I BEFORE INSERT ON RESOURCES FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  KEYTODELETE := CONCAT('*resourceswalluserid787ANDrownum5ORDERBYriddesc',:NEW.walluserid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  RESOURCES003891039104D BEFORE DELETE ON RESOURCES FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  KEYTODELETE := CONCAT('*resourceswalluserid787ANDrownum5ORDERBYriddesc',:OLD.walluserid);DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  RESOURCES003891039104U BEFORE UPDATE ON RESOURCES FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  IF (:OLD.RID <> :NEW.RID OR :OLD.CREATORID <> :NEW.CREATORID OR :OLD.WALLUSERID <> :NEW.WALLUSERID OR :OLD.TYPE <> :NEW.TYPE OR :OLD.BODY <> :NEW.BODY OR :OLD.DOC <> :NEW.DOC) THEN   KEYTODELETE := CONCAT('*resourceswalluserid787ANDrownum5ORDERBYriddesc',:OLD.walluserid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); KEYTODELETE := CONCAT('*resourceswalluserid787ANDrownum5ORDERBYriddesc',:NEW.walluserid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			
			// ViewComment
			stmt.execute("CREATE OR REPLACE TRIGGER  MANIPULATION005112339310I BEFORE INSERT ON MANIPULATION FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  " +
					"KEYTODELETE := CONCAT('"+ viewCommentKey() +"',:NEW.rid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  MANIPULATION005112339310D BEFORE DELETE ON MANIPULATION FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  " +
					"KEYTODELETE := CONCAT('"+ viewCommentKey() +"',:OLD.rid);DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
			stmt.execute("CREATE OR REPLACE TRIGGER  MANIPULATION005112339310U BEFORE UPDATE ON MANIPULATION FOR EACH ROW DECLARE COSAR_Ret_Val BINARY_INTEGER; KEYTODELETE  CLOB;  DELETEKEY CLOB; BEGIN DELETEKEY := ' '; KEYTODELETE := ' ';  IF (:OLD.MID <> :NEW.MID OR :OLD.CREATORID <> :NEW.CREATORID OR :OLD.RID <> :NEW.RID OR :OLD.MODIFIERID <> :NEW.MODIFIERID OR :OLD.TIMESTAMP <> :NEW.TIMESTAMP OR :OLD.TYPE <> :NEW.TYPE OR :OLD.CONTENT <> :NEW.CONTENT) THEN   " +
					"KEYTODELETE := CONCAT('"+ viewCommentKey() +"',:OLD.rid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); " +
					"KEYTODELETE := CONCAT('"+ viewCommentKey() +"',:NEW.rid); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); END IF; IF (DELETEKEY != ' ') THEN  COSAR_Ret_Val := COSARDeleteMultiCall('RAYS', DELETEKEY, 0); END IF; END;");
		} catch (Exception e) {
			e.printStackTrace(System.out);
		} finally {
			try {
