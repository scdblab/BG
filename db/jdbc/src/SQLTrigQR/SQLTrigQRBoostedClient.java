...
//author sumita barahmand, Jason Yap

public class SQLTrigQRBoostedClient extends DB implements JdbcDBClientConstants {

	public static final String CLIENT_NAME = "SQLTrigQRBoosted";
	private static boolean ManageCOSAR = false;
	private static boolean EAGERUPDATEENABLED = false;
	private static boolean verbose = false;
	private static String LOGGER = "ERROR, A1";	
	private static boolean connectionPoolInitialized = false;
	...
	//String cache_cmd_refresh = "C:\\PSTools\\psexec \\\\"+COSARServer.cacheServerHostname+" -u shahram -p 2Shahram C:\\cosar\\configurable\\TCache.NetworkInterface.exe C:\\cosar\\configurable\\V2gb_refresh.xml ";
	...
	private static int LOCK_EXCLUSIVE_STMT = 101;
	...
	private void cleanupAllConnections() {
		try {
			...
			if(lockStatement != null) lockStatement.close();
	...
	private String getCacheCmd(boolean refresh)
	{		
		if(refresh)
		{
			return "C:\\PSTools\\psexec \\\\"+cache_hostname+
				" -u shahram -p 2Shahram " +
				"C:\\cosar\\configurable\\TCache.NetworkInterface.exe C:\\cosar\\configurable\\V2gb_refresh.xml ";
		}
		else
		{
			return "C:\\PSTools\\psexec \\\\"+cache_hostname+
	...
	public boolean init() throws DBException {
		...
		EAGERUPDATEENABLED = Boolean.parseBoolean(props.getProperty(ENABLE_EAGERUPDATES_PROPERTY, ENABLE_EAGERUPDATES_PROPERTY_DEFAULT));
...
//			if((preparedStatement = newCachedStatements.get(LOCK_EXCLUSIVE_STMT)) == null){
//				preparedStatement = createAndCacheStatement(LOCK_EXCLUSIVE_STMT, "LOCK TABLE ? IN EXCLUSIVE MODE");
//			}
		...
		if (initialized || connectionPoolInitialized) {
			crtcl.release();
		...
		com.rays.cosar.COSARInterface.STRICT_CONSISTENCY_ENABLED = !EAGERUPDATEENABLED; //Gumball requires this to be true. For Eager updates though, it has to be false otherwise inserts will never make it into the cache
		...
		com.rays.sql.Statement.setTransparentCaching(true); //When true, it attempts to generate triggers and cache queries before using the developer provided re-writes.
		
		//com.rays.cosar.COSARInterface.USE_JAVA_SERIALIZATION = false;
		
		if (ManageCOSAR) {		
			System.out.println(CLIENT_NAME + " starting cache server");
		...
		System.out.println(CLIENT_NAME + " starting connection pool");
		...
		System.out.println(CLIENT_NAME + " pool started");
		...
		System.out.println(CLIENT_NAME + " triggers registered");
	...
	public void cleanup(boolean warmup) throws DBException {
		try {
			...
			if(!warmup){
				...
				//System.out.println(CLIENT_NAME + " db connection closed");
				
				//if (verbose) 
					System.out.println("Cleanup (after warmup-chk):  NumThreads="+NumThreads);
				if (NumThreads.get() > 0){
					crtcl.release();
					//cleanupAllConnections();
				...
				System.out.println(CLIENT_NAME + " shutting down pool");
				...
				System.out.println(CLIENT_NAME + " pool down");
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		...
		try {
			...
			common.CacheUtilities.ExecuteUpdateStmt(conn, proc, lockStatement, tables_to_lock);
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		//System.out.println("ListFriends " + requesterID);
	...
	public int viewFriendReq(int profileOwnerID,
		...
		//System.out.println("ViewFriendReq " + profileOwnerID);
	...
	public int viewTopKResources(int requesterID, int profileOwnerID, int k,
		...
		//System.out.println("ViewTopKResources " + profileOwnerID);

		String query = "SELECT * FROM resources WHERE walluserid = ? AND rownum <? ORDER BY rid desc";
		//String query = "SELECT * FROM resources WHERE walluserid = ?";
		try {
	...
	public int viewCommentOnResource(int requesterID, int profileOwnerID,
		...
		//System.out.println("ViewCommentOnResource " + profileOwnerID);
