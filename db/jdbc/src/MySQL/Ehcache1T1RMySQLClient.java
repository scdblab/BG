...
//
//
//public class Ehcache1T1RMySQLClient extends DB implements MySQLConstants {
//
...
//    private boolean isEhcacheModeEnabled;
//
...
//        isEhcacheModeEnabled = Boolean.parseBoolean(props.getProperty(EH_CACHE_MODE_PARAM,EH_CACHE_MODE_PARAM_DEFAULT));
//
...
//
//				//Shutdown Ehcache
//                if (isEhcacheModeEnabled) CacheManager.getInstance().shutdown();
//
//				crtcl.release();
//			}
//		} catch (Exception e) {
...
//                if (isEhcacheModeEnabled) CacheEntry.put("friendcount", Value.getBytes());
//				result.put("friendcount", new ObjectByteIterator(rs.getString(1).getBytes())) ;
//			}else {
//                if (isEhcacheModeEnabled) CacheEntry.put("friendcount", "0".getBytes());
...
//				if (rs.next()){
//					Value = rs.getString(1);
//                    if (isEhcacheModeEnabled) CacheEntry.put("pendingcount", Value.getBytes());
//					result.put("pendingcount", new ObjectByteIterator(rs.getString(1).getBytes())) ;
//				}
//				else {
//                    if (isEhcacheModeEnabled) CacheEntry.put("pendingcount", "0".getBytes());
...
//                if (isEhcacheModeEnabled) CacheEntry.put("resourcecount", rs.getString(1).getBytes());
//				result.put("resourcecount", new ObjectByteIterator(rs.getString(1).getBytes())) ;
//			} else {
//				//SR.put("resourcecount", new StringByteIterator("0")) ;
//                if (isEhcacheModeEnabled) CacheEntry.put("resourcecount", "0".getBytes());
...
//                        if (isEhcacheModeEnabled) CacheEntry.put(col_name,allBytesInBlob);
//						if(testMode){
//							//dump to file
//							try{
...
//                        if (isEhcacheModeEnabled) CacheEntry.put(col_name, value.getBytes());
...
//
//		String query = "SELECT * FROM resources WHERE walluserid = ? ORDER BY rid desc limit "+(k+1);
//		try {
