...
public class ApplicationCacheClient extends DB implements JdbcDBClientConstants {
	...
	private final byte[] deserialize_buffer = new byte[65536];
	...
	public boolean init() throws DBException {
		...
		com.rays.cosar.COSARInterface.setTTL(Integer.parseInt(props.getProperty(TTL_VALUE, TTL_VALUE_DEFAULT)));
		useTTL = Integer.parseInt(props.getProperty(TTL_VALUE, TTL_VALUE_DEFAULT)) != 0;
		...
		if (ManageCOSAR) {
			...
			this.st = new StartCOSAR(this.getCacheCmd(), "cache_output.txt");
			this.st.start();
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			...
			if (payload != null && CacheUtilities.unMarshallHashMap(result, payload, deserialize_buffer)){
...
//		}
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		try {
			...
			if (payload != null){
				if (verbose) System.out.println("... Cache Hit!");
				if (!CacheUtilities.unMarshallVectorOfHashMaps(payload,result,deserialize_buffer))
	...
	public int viewTopKResources(int requesterID, int profileOwnerID, int k,
		...
		try {
			cosar = CacheConnectionPool.getConnection(key);
			byte[] payload = cosar.retrieveBytesFromCache(key);
			if (payload != null && CacheUtilities.unMarshallVectorOfHashMaps(payload,result,deserialize_buffer)){
				if (verbose) System.out.println("... Cache Hit!");
				CacheConnectionPool.returnConnection(cosar, key);
				return retVal;
			}
	...
	public static void main(String[] args) {
		...
		for( int i = 0; i < CACHE_NUM_PARTITIONS; i++ )
		{
			CacheConnectionPool.addServer("127.0.0.1", 4343);
		}
