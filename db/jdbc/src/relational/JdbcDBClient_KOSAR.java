...
public class JdbcDBClient_KOSAR extends DB implements JdbcDBClientConstants {
	...
	private void cleanupAllConnections() {
		//		System.out.println("NumTokens: "+KosarSoloDriver.Tokens.size());
		...
		String s = "";
		s += "CacheHit: "+KosarSoloDriver.KosarCacheHitsEventMonitor.numberOfTotalEvents();
		s += "\nNumReads: "+KosarSoloDriver.KosarReadTimeMonitor.numberOfTotalEvents();
		s += "\nPercent Hit: "+(KosarSoloDriver.KosarCacheHitsEventMonitor.numberOfTotalEvents()/(double)KosarSoloDriver.KosarReadTimeMonitor.numberOfTotalEvents()); 
		s += "\nNumWrites: "+KosarSoloDriver.KosarDMLUpdateEventMonitor.numberOfTotalEvents();
		s += "\nNumILeaseGranted: "+KosarSoloDriver.KosarILeaseGrantedEventMonitor.numberOfTotalEvents();
		s += "\nNumILeaseBackoff: "+KosarSoloDriver.KosarReadBackoffEventMonitor.numberOfTotalEvents();
		s += "\nNumILeaseReleased: "+KosarSoloDriver.KosarILeaseReleasedEventMonitor.numberOfTotalEvents();		
		s += "\nNumQLeaseGranted: "+KosarSoloDriver.KosarQLeaseReleasedEventMonitor.numberOfTotalEvents();
		s += "\nNumQLeaseAborted: "+KosarSoloDriver.KosarQLeaseAbortEvntMonitor.numberOfTotalEvents();
		s += "\nNumEvitedKeys: "+KosarSoloDriver.KosarEvictedKeysEventMonitor.numberOfTotalEvents();

		PrintWriter out = null;
		try {
			out = new PrintWriter(new FileOutputStream("kosar_stats.txt", false));
			out.println(s);		    
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		...
		while (true) {
			try {
				...
				retVal = 0;
				break;
			} catch (SQLTimeoutException | SQLRecoverableException | KosarSQLException e) { 
				e.printStackTrace();
				cleanupAllConnections();
				try {
					while (!init());
		...
		if (requesterID == profileOwnerID) {
			...
			while (true) {
				try {
					...
					break;
				} catch (SQLTimeoutException | SQLRecoverableException | KosarSQLException ex) {
					ex.printStackTrace();
					cleanupAllConnections();
					try {
						while (!init());
