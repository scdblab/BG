...
public class MaterializedViewJdbcDBClient extends DB implements JdbcDBClientConstants {
	
	private static String FSimagePath = "";
	//private HashMap<String, byte[]> imageBytes = null;
	...
	private static int FRIENDREQ_REFRESH_STMT = 101;
	private static int PROFILE_REFRESH_STMT = 102;
	private static int LISTFRIENDS_REFRESH_STMT = 103;

	
	private boolean REFRESH_VIEWPROFILE_MV = false;
	private boolean REFRESH_LISTFRIENDS_MV = false;
	private boolean REFRESH_VIEWFRIENDREQ_MV = false;
	...
	public boolean init() throws DBException {
		...
		REFRESH_VIEWFRIENDREQ_MV = Boolean.parseBoolean(props.getProperty(REFRESH_VIEWFRIENDREQ_MV_PROP, REFRESH_VIEWFRIENDREQ_MV_DEFAULT));
		REFRESH_LISTFRIENDS_MV = Boolean.parseBoolean(props.getProperty(REFRESH_LISTFRIENDS_MV_PROP, REFRESH_LISTFRIENDS_MV_DEFAULT));
		REFRESH_VIEWPROFILE_MV = Boolean.parseBoolean(props.getProperty(REFRESH_VIEWPROFILE_MV_PROP, REFRESH_VIEWPROFILE_MV_DEFAULT));
		...
		try {
			...
			createAndCacheStatement(FRIENDREQ_REFRESH_STMT, 
					"BEGIN dbms_mview.refresh('ViewFriendReqMV', method=>'C'); END;");
			createAndCacheStatement(PROFILE_REFRESH_STMT, 
					"BEGIN dbms_mview.refresh('ViewProfileMV', method=>'C'); END;");
			createAndCacheStatement(LISTFRIENDS_REFRESH_STMT, 
					"BEGIN dbms_mview.refresh('ListFriendsMV', method=>'C'); END;");
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		...
		try {
			...
			if(REFRESH_VIEWFRIENDREQ_MV)
			{
				refreshStatement = newCachedStatements.get(FRIENDREQ_REFRESH_STMT);
				refreshStatement.executeUpdate();
			}
			
			if(REFRESH_LISTFRIENDS_MV)
			{
				refreshStatement = newCachedStatements.get(LISTFRIENDS_REFRESH_STMT);
				refreshStatement.executeUpdate();
			}
			
			if(REFRESH_VIEWPROFILE_MV)
			{
				refreshStatement = newCachedStatements.get(PROFILE_REFRESH_STMT);
				refreshStatement.executeUpdate();
			}
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			if(insertImage && FSimagePath.equals("")){
				query = "SELECT userid,username, fname, lname, gender, dob, jdate, ldate, address, email, tel, pic, pendcount, confcount, rescount FROM ViewProfileMV WHERE UserID = ?";
				...
				query = "SELECT userid,username, fname, lname, gender, dob, jdate, ldate, address, email, tel, pendcount, confcount, rescount FROM  ViewProfileMV WHERE UserID = ?";
...
//				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM ListFriendsMV WHERE ((inviterid=? and userid=inviteeid) or (inviteeid=? and userid=inviterid))";
				query = "SELECT userid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM ListFriendsMV WHERE targetuserid=?";
...
//				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM ListFriendsMV WHERE ((inviterid=? and userid=inviteeid) or (inviteeid=? and userid=inviterid))";
				query = "SELECT userid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM ListFriendsMV WHERE targetuserid=?";
...
//				query = "SELECT userid, inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM ViewFriendReqMV WHERE inviteeid=? and inviterid = userid";
				query = "SELECT userid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM ViewFriendReqMV WHERE targetuserid=?";
...
//				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM ViewFriendReqMV WHERE inviteeid=? and inviterid = userid";
				query = "SELECT userid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM ViewFriendReqMV WHERE targetuserid=?";
	...
	public void createSchema(Properties props){
		...
		try {
			...
			dropMaterializedView(stmt, "ViewProfileMV");
			dropMaterializedView(stmt, "ViewFriendReqMV");
			dropMaterializedView(stmt, "ListFriendsMV");
	...
	public void buildIndexes(Properties props){
		...
		try {
			...
			boolean isInsertImage = Boolean.parseBoolean(
					props.getProperty(Client.INSERT_IMAGE_PROPERTY, 
			...
			createMaterializedView(stmt, "CREATE MATERIALIZED VIEW ViewProfileMV "+
			...
			stmt.executeUpdate("CREATE UNIQUE INDEX ViewProfileMV_USERID ON ViewProfileMV (USERID)"
					+ " COMPUTE STATISTICS NOLOGGING");
			
			//*
			// Create the view used by both ListFriends and ViewFriendReq
			stmt.executeUpdate("CREATE OR REPLACE VIEW JoinedFriendsView AS "+
				"SELECT u.*,f.inviterid, f.inviteeid, f.status FROM users u, friendship f  "+
				"WHERE ((u.userid=f.inviteeid) or "+
				"  (u.userid=f.inviterid))");

			sql = "CREATE MATERIALIZED VIEW ViewFriendReqMV "+
				"REFRESH ON DEMAND AS  "+
				"SELECT u.userid AS targetuserid, f.userid AS userid,  "+
				"  f.username, f.pw, f.fname, f.lname, f.gender,  "+
				"  f.dob, f.jdate, f.ldate, f.address, f.email, f.tel ";
			if(isInsertImage)
			{
				sql += "  , f.tpic ";
			}
			sql += "FROM users u, JoinedFriendsView f  "+
				"WHERE ((f.inviterid=u.userid and f.userid=f.inviteeid) or  "+
				"  (inviteeid=u.userid and f.userid=f.inviterid)) "+
				"  AND f.status = 1 " +
				"ORDER BY targetuserid ";
			createMaterializedView(stmt, sql);
			  
			sql = "CREATE MATERIALIZED VIEW ListFriendsMV "+
				"REFRESH ON DEMAND AS  "+
				"SELECT u.userid AS targetuserid, f.userid AS userid,  "+
				"  f.username, f.pw, f.fname, f.lname, f.gender,  "+
				"  f.dob, f.jdate, f.ldate, f.address, f.email, f.tel ";
			...
			stmt.executeUpdate("CREATE INDEX ViewFriendReqMV_TARGETUSERID ON ViewFriendReqMV (TARGETUSERID)"
					+ " COMPUTE STATISTICS NOLOGGING");			  
			
			stmt.executeUpdate("CREATE INDEX ListFriendsMV_TARGETUSERID ON ListFriendsMV (TARGETUSERID)"
					+ " COMPUTE STATISTICS NOLOGGING");
			//*/
	...
	public static void dropMaterializedView(Statement st, String mvName)
	{
		try {
			st.executeUpdate("drop materialized view " + mvName);
		} catch (SQLException e) {
		}
	}
