...
public class AggrAsAttr1Rec2TableJdbcDBClient extends DB implements JdbcDBClientConstants {
	...
	public int queryConfirmedFriendshipIds(int profileId, Vector<Integer> confirmedIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT friend1, friend2 from confirmedfriendship where (friend1="+profileId+" OR friend2="+profileId+")";
			rs = st.executeQuery(query);
			while(rs.next()){
