 ...
 *
 */
public class NotInlineJdbcDBClient extends DB implements JdbcDBClientConstants {
        ...
        public int insertEntity(String entitySet, String entityPK, HashMap<String, ByteIterator> values, boolean insertImage) {
                ...
                try {
                        ...
                        System.out.println("Error in processing insert to table: " + entitySet +"\n"+ e);
                        return -2;
                } finally{
                        try {
                                if (rs != null)
                ...
                try {
                        ...
                        if (Boolean.parseBoolean(props.getProperty(Client.INSERT_IMAGE_PROPERTY,
                                ...
                                if (INLINE){
                                        ...
                                        //Drop tables spaces for profile and thumbnail images
                                        StmtWithoutException(stmt,"drop tablespace ProfileImages INCLUDING CONTENTS AND DATAFILES");
                                        StmtWithoutException(stmt,"drop tablespace ThumbnailImages INCLUDING CONTENTS AND DATAFILES");
                                       
                                        //Create table spaces for profile and thumbnail images.
                                        StmtWithoutException(stmt, "create tablespace ProfileImages NOLOGGING "
                            + "datafile 'C:/profileimages.db' "
                            + "SIZE 60m "
                            + "AUTOEXTEND ON "
                            + "NEXT 50m MAXSIZE 10240m "
                            + "EXTENT MANAGEMENT LOCAL");
                                        StmtWithoutException(stmt, "create tablespace ThumbnailImages NOLOGGING "
                            + "datafile 'C:/thumbnailimages.db' "
                            + "SIZE 30m "
                            + "AUTOEXTEND ON "
                            ...
                            + "EXTENT MANAGEMENT LOCAL");
                                        ...
                                        stmt.executeUpdate("CREATE TABLE USERS"
                                                        ...
                                                        + ") "
                                                        + "LOB (PIC) "
                                                        + "  STORE AS PIC_file_lob_seq ( "
                                                        + "     TABLESPACE ProfileImages "
                                                        + "     CHUNK 10240 "
                                                        + "     DISABLE STORAGE IN ROW "
                                                        + "  ) "
                                                        + "LOB (TPIC) "
                                                        + "  STORE AS THUMB_file_lob_seq ( "
                                                        + "     TABLESPACE ThumbnailImages "
                                                        + "     CHUNK 2048 "
                                                        + "     DISABLE STORAGE IN ROW "
                                                        + "  ) "
                                                        + "NOLOGGING");
       ...
       @Override
        ...
        public static void StmtWithoutException(Statement st, String cmd) {
                try {
