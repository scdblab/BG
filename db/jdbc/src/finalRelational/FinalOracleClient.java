 ...
 * with BG. This class extends {@link DB} and implements the database
 * interface used by BG client.
 * 
 * <br>
...
public class FinalOracleClient extends DB implements JdbcDBClientConstants {
		...
		try {
			...
			if (entitySet.equalsIgnoreCase("users") && insertImage) {
				...
				if (FSimagePath.equals("")) {
					preparedStatement.setBinaryStream(cnt, is,
							profileImage.length);
					cnt++;
				} else
			...
			if (entitySet.equalsIgnoreCase("users")) {
				...
				String updateQ = "Update users set rescount=rescount+1 where userid="
						+ values.get("walluserid");
	...
	public void createSchema(Properties props) {
		...
		try {
			...
			stmt.execute("CREATE OR REPLACE PROCEDURE INVITEFRIEND " + "("
					...
					+ "AS " + "BEGIN " + "INSERT INTO FRIENDSHIP " + "("
					+ "INVITERID     , " + "INVITEEID     , " + "STATUS" + ")"
					+ " VALUES " + "(" + "P_INVITERID     , "
					+ "P_INVITEEID     ," + "1" + ");" + "UPDATE USERS SET "
					+ "PENDCOUNT=PENDCOUNT+1 WHERE USERID = P_INVITEEID;"
					+ "COMMIT;" + "END INVITEFRIEND;");
